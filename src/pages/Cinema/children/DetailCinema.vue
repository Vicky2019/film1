  ...
  <div id="cinema-detail">
    <div class="cinema-info">
      ...
      <span class="address">
        ...
        {{currentCinemaInfo.specified_address}}
      </span>
      <span class="tel">
        <span class="icon icon-tel"></span>
        {{currentCinemaInfo.cinema_phone}}
      </span>
    </div>
    <div class="movie-container">
      ...
      >
        <el-carousel-item v-for="(item,index) in hasMovieInfo" :key="index">
          <a
            href="#"
            @click.prevent="$router.push({path:'/detail_movie',query:{movie_id:item.movie_id}})"
          >
            <img :src="'https://images.weserv.nl/?url='+item.poster.substring(7)" alt />
          </a>
        </el-carousel-item>
      ...
      >
        <span class="main">
          ...
          <span class="score">
            ...
            <span v-else style="font-size: .28rem">暂无评分</span>
          </span>
    ...
    <div class="ticket-container">
      <div class="item" v-for="(item,index) in movieDaySchedule" :key="index">
        ...
        <div class="right">
          <span
            class="buy-btn"
            @click="$router.push({path:'/select_seat',query:{cinema_id:item.cinema_id,movie_id:item.movie_id,schedule_id:item.schedule_id,}})"
          >购票</span>
        </div>
      </div>
    </div>
  ...
  methods: {
      ...
      if (this.$route.query.cinema_id) {
        ...
        if (json.success_code === 200) {
          ...
          movieScheduleInfo.forEach((value, index) => {
            ...
            value.forEach(val => {
              if (
                new Date() - new Date(val.show_date + "," + val.show_time) <=
                0
              ) {
                let flag = true;
          ...
          this.allMovieSchedule[0].movieInfo.forEach(value => {
            if (
              value.show_date === this.allMovieSchedule[0].movieDate[0].date
            ) {
          ...
          this.movieDaySchedule.sort((a, b) => {
            return (
              new Date(a.show_date + "," + a.show_time) -
              new Date(b.show_date + "," + b.show_time)
            );
          });
        }
      }
    ...
    changeCarousel(index) {
      ...
      this.allMovieSchedule[index].movieInfo.forEach(value => {
        if (
          value.show_date === this.allMovieSchedule[index].movieDate[0].date
        ) {
          this.movieDaySchedule.push(value);
        }
      });
...
#cinema-detail {
  ...
  .movie-container {
    .el-carousel {
      ...
      box-shadow: 0.02rem 0.02rem 0.08rem #242342;
      border-radius: 10px;
      width: 314px;
      float:left;
      margin-right:1rem;
